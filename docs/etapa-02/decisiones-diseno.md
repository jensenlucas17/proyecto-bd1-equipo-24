...
## Nota de consistencia con la Etapa 1

Los documentos de la Etapa 1 describen `Detalle_Venta` con los atributos `monto` y `monto_historico`. En el modelo relacional esa tabla pasa a llamarse `CONTIENE` y esos atributos se reemplazan por `precio_unitario` (más `cantidad`). El sentido de RN06 se mantiene; solo cambió su representación.
